...
from torchvision.ops.boxes import nms, box_iou, batched_nms
    ...
    if labels is not None:
        ...
        if pos.any():
            ...
            loss[pos] = F.binary_cross_entropy_with_logits(
                pos_pred, target_onehot, reduction='none') * (pos_scores.unsqueeze(1) - pred_sigmoid[pos]).pow(beta)
...
def giou_loss(pred, target, weight=None, reduction='none', avg_factor=None, eps=1e-7):
    ...
    giou = iou - (enclosed_wh[:, 0] * enclosed_wh[:, 1] - union) / (enclosed_wh[:, 0] * enclosed_wh[:, 1] + eps)
    loss = 1 - giou
...
class DynamicSoftLabelAssigner(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, pred_scores, priors, decoded_bboxes, gt_bboxes, gt_labels, gt_bboxes_ignore=None):
        ...
        if num_gt == 0 or num_bboxes == 0:
            return assigned_gt_inds, assigned_labels, max_overlaps, \
                   torch.zeros(0, dtype=torch.long), torch.zeros(0, dtype=torch.long), 0

        # Check if prior center is inside gt
        prior_center = priors[:, :2]
        deltas = torch.cat([prior_center[:, None] - gt_bboxes[:, :2],
                           gt_bboxes[:, 2:] - prior_center[:, None]], dim=-1)
        ...
        return assigned_gt_inds, assigned_labels, max_overlaps, pos_inds, neg_inds, pos_inds.size(0)
...
class ConvModule(nn.Module):
    ...
    def __init__(self, in_c, out_c, k=1, s=1, p=0, groups=1, depthwise=False, act=nn.LeakyReLU(0.1, inplace=True)):
        super().__init__()
        self.act = act
...
class GhostPAN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=FPN_OUT_CHANNELS, kernel_size=KERNEL_SIZE,
                 num_extra_level=NUM_EXTRA_LEVEL, act=nn.LeakyReLU(0.1, inplace=True)):
        super().__init__()
        ...
        self.downsamples = nn.ModuleList([
            ConvModule(out_channels, out_channels, kernel_size, 2, kernel_size//2, depthwise=True, act=act)
            for _ in range(len(in_channels) - 1)
        ])
...
class NanoDetPlusHead(nn.Module):
    ...
    def __init__(self, num_classes, input_channel=FPN_OUT_CHANNELS, feat_channels=FPN_OUT_CHANNELS,
                 stacked_convs=STACKED_CONVS, kernel_size=KERNEL_SIZE, strides=STRIDES, reg_max=REG_MAX):
        super().__init__()
        ...
        for _ in strides:
            cls_convs = nn.ModuleList([
                ConvModule(self.in_channels if i == 0 else self.feat_channels,
                          self.feat_channels, kernel_size, 1, kernel_size//2, depthwise=True, act=act)
                for i in range(stacked_convs)
            ])
            self.cls_convs.append(cls_convs)
            self.gfl_cls.append(nn.Conv2d(self.feat_channels,
                                         num_classes + 4 * (reg_max + 1), 1))
    ...
    def loss(self, preds, gt_meta, aux_preds=None):
        ...
        input_height, input_width = gt_meta[0]['img'].shape[2:] if 'img' in gt_meta[0] else (416, 416)
        mlvl_center_priors = [
            self.get_single_level_center_priors(batch_size,
                (math.ceil(input_height / s), math.ceil(input_width / s)), s,
        ...
        if aux_preds is not None:
            ...
            batch_assign_res = self.target_assign(
                aux_cls_preds.detach(), center_priors, aux_decoded_bboxes.detach(), gt_bboxes, gt_labels)
        else:
            batch_assign_res = self.target_assign(
                cls_preds.detach(), center_priors, decoded_bboxes.detach(), gt_bboxes, gt_labels)
    ...
    def _get_loss_from_assign(self, cls_preds, reg_preds, decoded_bboxes, assign):
        ...
        loss_qfl = quality_focal_loss(cls_preds, (labels, label_weights), reduction='sum') / num_total_samples
        ...
        if len(pos_inds) > 0:
            ...
            loss_bbox = giou_loss(decoded_bboxes[pos_inds], bbox_targets[pos_inds],
                                 weight=weight_targets, reduction='sum', avg_factor=bbox_avg_factor)
            loss_dfl = distribution_focal_loss(
                reg_preds[pos_inds].reshape(-1, self.reg_max + 1),
                dist_targets[pos_inds].reshape(-1),
                weight=weight_targets[:, None].expand(-1, 4).reshape(-1),
                reduction='sum', avg_factor=4.0 * bbox_avg_factor)

        return loss_qfl + loss_bbox + loss_dfl, dict(loss_qfl=loss_qfl, loss_bbox=loss_bbox, loss_dfl=loss_dfl)

    def target_assign(self, cls_preds, center_priors, decoded_bboxes, gt_bboxes, gt_labels):
        """Assign targets for a batch of images."""
        all_labels, all_label_weights, all_bbox_targets, all_dist_targets, num_pos_total = [], [], [], [], 0

        for i in range(cls_preds.shape[0]):
            labels, label_weights, bbox_targets, dist_targets, num_pos = self.target_assign_single_img(
                cls_preds[i], center_priors[i], decoded_bboxes[i], gt_bboxes[i], gt_labels[i])
        ...
        return all_labels, all_label_weights, all_bbox_targets, all_dist_targets, num_pos_total

    def target_assign_single_img(self, cls_preds, center_priors, decoded_bboxes, gt_bboxes, gt_labels):
        """Compute targets for one image."""
        if len(gt_bboxes) == 0:
            num_priors = center_priors.size(0)
            return (center_priors.new_full((num_priors,), self.num_classes, dtype=torch.long),
                   center_priors.new_zeros(num_priors, dtype=torch.float),
                   torch.zeros_like(center_priors), torch.zeros_like(center_priors), 0)

        assigned_gt_inds, assigned_labels, max_overlaps, pos_inds, neg_inds, num_pos = self.assigner(
        ...
        if len(pos_inds) > 0:
            ...
            dist_targets[pos_inds, :] = bbox2distance(center_priors[pos_inds, :2], pos_gt_bboxes) / strides
...
class SimpleConvHead(nn.Module):
    ...
    def __init__(self, num_classes, input_channel=AUX_FEAT_CHANNELS, feat_channels=AUX_FEAT_CHANNELS,
                 stacked_convs=AUX_STACKED_CONVS, strides=STRIDES, reg_max=REG_MAX):
        super().__init__()
        self.num_classes = num_classes
        self.strides = strides
        self.reg_max = reg_max
    ...
    def forward(self, feats):
        ...
        for i, x in enumerate(feats):
            ...
            outputs.append(torch.cat([self.gfl_cls(cls_feat), self.gfl_reg(reg_feat)], dim=1).flatten(start_dim=2))
        return torch.cat(outputs, dim=2).permute(0, 2, 1)
...
class NanoDetPlus(BaseObjectDetection):
    ...
    def __init__(self, config):
        ...
        # Backbone
        self.model.backbone = ShuffleNetV2(model_size="1.0x", out_stages=(2, 3, 4), pretrain=load_backbone_pretrained)
    ...
    def load_nanodet_weights(self, model_url):
        ...
        print(f"  - Loaded: {len(new_state_dict) - len([k for k in new_state_dict.keys() if k in msg.missing_keys])} keys")
    ...
    def forward(self, inputs, targets=None):
        ...
        center_priors = torch.cat([
            self.model.head.get_single_level_center_priors(batch_size, featmap_sizes[i], s,
        ...
        if self.model.training and targets is not None:
            ...
            return {
                ...
                "aux_loss": loss_states.get('aux_loss_qfl', loss * 0) + loss_states.get('aux_loss_bbox', loss * 0),
                "total_loss": loss
            }
        else:
            ...
            for i in range(batch_size):
                ...
                keep = batched_nms(masked_boxes, masked_scores, masked_labels,
