...
class TerraMindModule(nn.Module):
    """TerraMind model.
    ...
    def cat_decoder_tensors(
        ...
        for mod, d in mod_dict.items():
            if self.modality_info[mod]["type"] in ["seq", "seq_emb", "seq_token"]:
                ...
                mod_mask_all.append(
                    torch.full_like(
                        d["ids"][:, :-1], self.modality_info[mod]["id"], dtype=torch.int
                    )
                )
            else:
    ...
    def adapt_decoder_attention_mask(
        self, decoder_attention_mask: torch.Tensor, mod_mask=torch.Tensor | None
    ) -> torch.Tensor:
        """
        ...
        for mod, d in decoder_mod_dict.items():
            ...
            if logits.numel() == 0:
                ...
                loss = F.cross_entropy(
                    logits, target_ids[decoder_mod_mask == idx].long(), reduction="mean"
                )
                mod_loss[mod] = loss
...
def build_output_modality_embeddings(
    modality_info, modalities, img_size=None, dim=None, patch_size=None
):
    mod_embeddings = {}
    mod_name_mapping = {}
