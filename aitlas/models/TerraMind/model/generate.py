...
def empty_seq_emb_modality(mod_dict, mod=None):
...
def init_conditioned_target_modality(
    mod_dict, modality_info, domain, num_target_tokens, eos_id=3, s1_id=5
):
...
def init_full_input_modality(value, modality_info, domain, device, eos_id=3):
    ...
    if "input_mask" not in mod_dict:
        mod_dict["input_mask"] = torch.zeros(
            mod_dict["tensor"].shape, dtype=torch.bool, device=device
        )
    if "target_mask" not in mod_dict:
        mod_dict["target_mask"] = torch.ones(
            mod_dict["tensor"].shape, dtype=torch.bool, device=device
        )
