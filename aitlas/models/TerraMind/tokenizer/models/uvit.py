...
class Block(nn.Module):
    ...
    def forward(self, x, temb=None, mask=None, skip_connection=None):
        gate_msa, gate_mlp = (
            self.adaLN_gate(F.silu(temb)).unsqueeze(1).chunk(2, dim=-1)
            if hasattr(self, "adaLN_gate")
            else (1.0, 1.0)
        )
        shift_msa, scale_msa, shift_mlp, scale_mlp = (
            self.adaLN_modulation(F.silu(temb)).chunk(4, dim=-1)
            if hasattr(self, "adaLN_modulation")
            else 4 * [0.0]
        )
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, context, temb=None, sa_mask=None, xa_mask=None, skip_connection=None):
        ...
        shift_msa, scale_msa, shift_mxa, scale_mxa, shift_mlp, scale_mlp = (
            self.adaLN_modulation(F.silu(temb)).chunk(6, dim=-1)
            if hasattr(self, "adaLN_modulation")
            else 6 * [0.0]
        )
        if self.skip_linear is not None:
...
class TransformerConcatCond(nn.Module):
    ...
    def forward(
        ...
        if cond_mask is not None:
            cond_mask = (
                F.interpolate(cond_mask.unsqueeze(1).float(), (H_mid, W_mid), mode="nearest") > 0.5
            )
...
class TransformerXattnCond(nn.Module):
    ...
    def forward(
        ...
        xa_mask = (
            repeat(cond_mask, "b h w -> b n (h w)", n=x.shape[1]) if cond_mask is not None else None
        )
        ...
        **kwargs,
        ...
        if orig_res is not None and self.res_embedding:
            if not torch.is_tensor(orig_res):
                ...
                dtype = torch.int if is_mps else torch.int64
                h_orig = torch.tensor([h_orig], dtype=dtype, device=sample.device).expand(
                    sample.shape[0]
                )
                w_orig = torch.tensor([w_orig], dtype=dtype, device=sample.device).expand(
                    sample.shape[0]
                )
            else:
