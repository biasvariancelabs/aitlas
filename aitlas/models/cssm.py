...
class MambaBlock_CD(nn.Module):
    ...
    def cssm(self, t1, t2):
        ...
        (delta_prim, B_prim, C_prim) = t2_dbl.split(
            split_size=[self.dt_rank, n, n], dim=-1
        )  # delta_prim: (b, l, dt_rank). B, C: (b, l, n)
        delta_prim = F.softplus(self.dt_proj(delta_prim))  # (b, l, d_in)

        y = self.selective_scan(
            t1, t2, delta, delta_prim, A, B, C, D, A_prim, B_prim, C_prim, D_prim
        )
        return y
...
class CSSMModel(nn.Module):
    ...
    def __init__(self, in_channels=3, output_classes=2, img_size=256):
        ...
        self.conv_4 = nn.Sequential(nn.Conv2d(256, 128, 3, 1, padding=1), nn.ReLU())
        ...
        self.conv_3 = nn.Sequential(nn.Conv2d(128, 64, 3, 1, padding=1), nn.ReLU())
        ...
        self.conv_2 = nn.Sequential(nn.Conv2d(64, 32, 3, 1, padding=1), nn.ReLU())
        ...
        self.conv_1 = nn.Sequential(nn.Conv2d(32, 16, 3, 1, padding=1), nn.ReLU())
