...
class iRPE(nn.Module):
    ...
    def forward(self, x, height=None, width=None, pos=None, modis=False):
        ...
        rp_bucket, self._ctx_rp_bucket_flatten = self._get_rp_bucket(
            x, height=height, width=width, pos=pos, modis=modis
        )
        if self.transposed:
            ...
            rp_bucket = torch.cat(
                ...
                + [
                    torch.gather(
                        rp_bucket[:, skip:, :],
                        dim=1,
                        index=pos.unsqueeze(-1).repeat(1, 1, rp_bucket.shape[2]),
                    )
                ],
                dim=1,
            )
            rp_bucket = torch.cat(
                ...
                + [
                    torch.gather(
                        rp_bucket[:, :, skip:],
                        dim=2,
                        index=pos.unsqueeze(1).repeat(1, rp_bucket.shape[1], 1),
                    )
                ],
                dim=2,
            )
    ...
    def forward_rpe_transpose(self, x, rp_bucket):
        ...
        if self.mode == "bias":
            ...
            if RPEIndexFunction is not None:
                ...
                look = torch.gather(
                    look, dim=2, index=self._ctx_rp_bucket_flatten.unsqueeze(1)
