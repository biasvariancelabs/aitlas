...
from .utils import DropPath, Mlp
    ...
    def __init__(
        ...
        if use_flash_attn:
            try:
                ...
                raise ImportError(
                    "flash-attn is not installed. Please install it with `pip install flash-attn`"
                )
        else:
...
class RPEAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        B, N, C = x.shape
        if mask is None:
            height = int((N // self.n_modalities) ** 0.5)
        else:
            height = mask.shape[-1]
...
class CrossRPEAttention(nn.Module):
    def __init__(
        ...
        self.pos_embed = get_2d_sincos_pos_embed_with_scale(
            dim, int(num_patches**0.5), cls_token=True, scale=scale, modis=modis
        )
        ...
        self.rpe_q, self.rpe_k, self.rpe_v = build_rpe(
            rpe_config, head_dim=self.head_dim, num_heads=num_heads, n_modalities=1
        )
    ...
    def forward(self, x, mask=None):
        ...
        if mask is None:
            ...
            masked_pos_embed = torch.gather(
                self.pos_embed.expand(B, -1, -1)[:, 1:], dim=1, index=mask_pos
            )
...
class CrossRPEAttentionMulti(nn.Module):
    def __init__(
        ...
        if not release:
            ...
            for dataset in self.datasets:
                self.len_modalities[dataset] = len(modalities[dataset]) - int(
                    "modis" in modalities[dataset]
                )
        ...
        self.rpe_q, self.rpe_k, self.rpe_v = build_rpe(
            rpe_config, head_dim=head_dim, num_heads=num_heads, n_modalities=1
        )
    ...
    def forward(self, x, mask=None, dataset="", scale=1):
        ...
        if mask is None:
            num_patches = (
                N // self.len_modalities[dataset]
                + int(N % self.len_modalities[dataset] > 0)
                + modis
            )
        ...
        x = (
            (attn @ v).transpose(1, 2).reshape(B, num_patches, C)
    ...
    def forward_release(self, x, mask=None, n_modalities=1, modis=False, scale=1):
        ...
        if self.rpe_k is not None:
            ...
            attn += torch.cat(
                [
                    rpe[:, :, :, : (1 + modis)],
                    rpe[:, :, :, (1 + modis) :].repeat(1, 1, 1, n_modalities),
                ],
                dim=-1,
            )
...
class CrossBlockMulti(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = CrossRPEAttentionMulti(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
    ...
    def forward_release(self, x, n_modalities=1, modis=False, scale=1):
        x = self.drop_path(
            self.attn.forward_release(
                self.norm1(x), n_modalities=n_modalities, modis=modis, scale=scale
            )
        )
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
