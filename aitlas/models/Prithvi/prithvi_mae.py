        ...
        **kwargs,
        ...
        if self.vpt:
            ...
            self.vpt_prompt_embeddings = nn.ParameterList(
                [
                    nn.Parameter(torch.zeros(1, self.vpt_n_tokens, embed_dim))
    ...
    def initialize_weights(self):
        ...
        if self.vpt:
            ...
            val = np.sqrt(
                6.0
                / float(
                    3 * reduce(mul, self.patch_embed.patch_size[1:], 1) + self.embed_dim
                )
            )
        ...
        for idx, block in enumerate(self.blocks):
            if self.vpt:
                x = torch.cat(
                    (
                        x[:, :1, :],
                        self.vpt_dropout_layers[idx](
                            self.vpt_prompt_embeddings[idx].expand(bs, -1, -1)
                        ),
                        x[:, 1:, :],
                    ),
                    dim=1,
    ...
    def forward(
        ...
        if self.temporal_encoding and temporal_coords is not None:
            num_tokens_per_frame = (
                x.shape[1] // self.num_frames
            )  # TODO: fix the decoder to process more than num_frames=4 time steps
...
def prithvi_eo_v1_base_model(**kwargs):
    config = _cfg(num_frames=3, mean=PRITHVI_V1_MEAN, std=PRITHVI_V1_STD)
...
def prithvi_eo_v2_base_tl_model(**kwargs):
    config = _cfg(coords_encoding=["time", "location"], coords_scale_learn=True)
    model = PrithviMAE(**config)
    return model


def prithvi_eo_v2_large_model(**kwargs):
    config = _cfg(embed_dim=1024, depth=24, num_heads=16)
    model = PrithviMAE(**config)
    return model
...
def prithvi_eo_v2_huge_model(**kwargs):
    config = _cfg(embed_dim=1280, depth=32, num_heads=16, patch_size=[1, 14, 14])
    model = PrithviMAE(**config)
    return model
