...
class AnchorGenerator(nn.Module):
    ...
    def forward(self, feature_maps):
        ...
        for size, stride, feat in zip(self.anchor_sizes, self.anchor_strides, feature_maps):
            ...
            shift_y, shift_x = torch.meshgrid(shifts_y, shifts_x, indexing='ij') if int(torch.__version__.split('.')[1]) >= 10 else torch.meshgrid(shifts_y, shifts_x)
            shift_x = shift_x.reshape(-1)
            shift_y = shift_y.reshape(-1)
            shifts = torch.stack((shift_x, shift_y, shift_x, shift_y), dim=1) + stride // 2
            base_anchor = torch.tensor([-size/2, -size/2, size/2, size/2], dtype=torch.float32, device=device)
...
class BackboneWithFPN(nn.Module):
    ...
    def __init__(self, backbone_name, pretrained=True, in_channels=3):
        ...
        in_channels_list = [512, 1024, 2048] if backbone_name not in ['resnet18', 'resnet34'] else [128, 256, 512]
...
class ATSSHead(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, num_anchors=1):
        ...
        self.centerness = nn.Conv2d(in_channels, num_anchors * 1, kernel_size=3, stride=1, padding=1)
...
class ATSSModel(nn.Module):
    def __init__(self, num_classes, pretrained=True, backbone_name='resnet50', in_channels=3):
        super().__init__()
        self.backbone = BackboneWithFPN(backbone_name, pretrained=pretrained, in_channels=in_channels)
...
def compute_atss_targets(anchors, gt_boxes, gt_labels, num_anchors_per_level, topk=9):
    ...
    if num_gt == 0:
        return torch.full((num_anchors,), -1, dtype=torch.int64, device=anchors.device), \
...
class ATSS(BaseObjectDetection):
    ...
    def load_atss_weights(self, checkpoint_path):
        ...
        if not os.path.exists(checkpoint_path):
            ...
            state_dict = checkpoint_path if isinstance(checkpoint_path, dict) else torch.load(checkpoint_path, map_location=self.device)
        else:
        ...
        for k, v in state_dict.items():
            ...
            if self.num_classes != 80 and ("cls_logits.weight" in name or "cls_logits.bias" in name):
    ...
    def forward(self, inputs, targets=None):
        if isinstance(inputs, list):
            target_sizes = torch.stack([torch.tensor([img.shape[-2], img.shape[-1]], device=self.device) for img in inputs])
            samples = nested_tensor_from_tensor_list(inputs).to(self.device)
        else:
            target_sizes = torch.stack([torch.tensor([inputs.shape[-2], inputs.shape[-1]], device=self.device)] * inputs.shape[0])
        ...
        if self.model.training and targets is not None:
            ...
            logits_concat = torch.cat([l.permute(0, 2, 3, 1).reshape(B, -1, self.num_classes) for l in logits], dim=1)
            bbox_reg_concat = torch.cat([b.permute(0, 2, 3, 1).reshape(B, -1, 4) for b in bbox_reg], dim=1)
            centerness_concat = torch.cat([c.permute(0, 2, 3, 1).reshape(B, -1) for c in centerness], dim=1)
            ...
            for i in range(B):
                ...
                target_labels, target_boxes = compute_atss_targets(
                    anchors_concat, gt_boxes, gt_labels, num_anchors_per_level, topk=self.model.topk
                )
            ...
            for i in range(B):
                ...
                if pos_inds.sum() > 0:
                    cent_t = compute_centerness_targets(target_boxes_batch[i], anchors_concat[pos_inds])
            ...
            for i in range(B):
                pos_inds = pos_inds_batch[i]
                if pos_inds.sum() > 0:
                    decoded_preds = self.model.box_coder.decode(bbox_reg_concat[i][pos_inds], anchors_concat[pos_inds])
                    r_loss = giou_loss(decoded_preds, target_boxes_batch[i], weight=centerness_targets_all[i])
                    reg_loss += r_loss
                    
                    c_loss = F.binary_cross_entropy_with_logits(centerness_concat[i][pos_inds], centerness_targets_all[i], reduction="sum")
            ...
            for i in range(B):
                ...
                for level in range(len(logits)):
                    ...
                    l_boxes_pred = self.model.box_coder.decode(l_bbox_reg[box_loc], l_anchors[box_loc])
