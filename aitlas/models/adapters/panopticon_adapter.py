...
from .utils import SENTINEL_1_IDS, SENTINEL_2_WAVELENGTHS, LANDSAT_8_WAVELENGTHS
            ...
            if has_s1:
                for band in bands_s1:
                    if band not in SENTINEL_1_IDS:
                        raise ValueError(f"Invalid SAR band {band}. Must be one of {list(SENTINEL_1_IDS.keys())}.")
                    chn_list.append(SENTINEL_1_IDS[band])
                    
            if has_s2:
                for band in bands_s2:
                    if band not in SENTINEL_2_WAVELENGTHS:
                        raise ValueError(f"Invalid Optical band {band}. Must be one of {list(SENTINEL_2_WAVELENGTHS.keys())}.")
                    chn_list.append(float(1000 * SENTINEL_2_WAVELENGTHS[band]))

            if has_l8:
                for band in bands_l8:
                    if band not in LANDSAT_8_WAVELENGTHS:
                        raise ValueError(f"Invalid Optical band {band}. Must be one of {list(LANDSAT_8_WAVELENGTHS.keys())}.")
