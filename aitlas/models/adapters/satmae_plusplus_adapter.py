...
@ADAPTER_REGISTRY.register("SatMAEPlusPlusAdapter")
class SatMAEPlusPlusAdapter(BaseInputAdapter):
    def forward(self, x: torch.Tensor):
        ...
        if isinstance(x, torch.Tensor):
            ...
            if "multispectral" in backbone_name:
                ...
                # Enforce spatial dimensions (96x96)
                if x.shape[-2:] != (96, 96):
                    data_transforms = v2.Resize(96, antialias=True)
                    x = data_transforms(x)
                    
            # Standard RGB backbone
            else:
