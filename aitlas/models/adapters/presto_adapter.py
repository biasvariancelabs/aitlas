    ...
    def forward(self, x):
        if isinstance(x, torch.Tensor):
            ...
            if has_s1 and set(bands_s1) != set(PRESTO_S1_BANDS):
                raise ValueError(
                    f"Presto requires exactly these S1 bands: {PRESTO_S1_BANDS}. Got: {bands_s1}"
                )
            if has_s2 and set(bands_s2) != set(PRESTO_S2_BANDS):
                raise ValueError(
                    f"Presto requires exactly these S2 bands: {PRESTO_S2_BANDS}. Got: {bands_s2}"
                )
            ...
            inputs_dict["latlons"] = torch.zeros(
                (batch_size, 2, H, W), dtype=torch.float32, device=x.device
            )

            # Inject the required keyword arguments for Presto
            kwargs = {"inputs": inputs_dict}
