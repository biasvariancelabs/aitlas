...
class AnySatAdapter(BaseInputAdapter):
    def forward(self, x):
        ...
        if isinstance(x, torch.Tensor):
            ...
            ANYSAT_S2_BANDS = ["B02", "B03", "B04", "B05", "B06", "B07", "B08", "B8A", "B11", "B12"]
            ANYSAT_L8_BANDS = ["B01", "B02", "B03", "B04", "B05", "B06", "B07", "B08", "B09", "B10", "B11"]
            ...
            if has_s1 and set(bands_s1) != set(ANYSAT_S1_BANDS):
                raise ValueError(f"AnySat requires exactly these S1 bands: {ANYSAT_S1_BANDS}. Got: {bands_s1}")
            if has_s2 and set(bands_s2) != set(ANYSAT_S2_BANDS):
                raise ValueError(f"AnySat requires exactly these S2 bands: {ANYSAT_S2_BANDS}. Got: {bands_s2}")
            if has_l8 and set(bands_l8) != set(ANYSAT_L8_BANDS):
                raise ValueError(f"AnySat requires exactly these L8 bands: {ANYSAT_L8_BANDS}. Got: {bands_l8}")
