...
from .Prithvi.prithvi_mae import (
    PrithviMAE,
    PrithviViT,
    prithvi_eo_v1_base,
...
class Prithvi(FoundationModel):
    ...
    def forward_features(
        ...
        **kwargs,
        ...
        embedding = self.backbone.forward_features(
            x, temporal_coords, location_coords, **kwargs
        )

        # Check if the features for segmentation should be output
        if segmentation_features == True:
            embedding = self.backbone.encoder.prepare_features_for_image_model(
                embedding
            )
