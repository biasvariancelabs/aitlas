...
class RCNNHead(nn.Module):
    def __init__(self, d_model, num_classes, dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu",
                 num_cls=1, num_reg=3, dim_dynamic=64, num_dynamic=2, pooler_resolution=7, scale_clamp=math.log(100000.0 / 16)):
        super().__init__()
...
class DynamicHead(nn.Module):
    def __init__(self, num_classes, hidden_dim, dim_feedforward, nhead, dropout, activation, num_heads, num_cls, num_reg, dim_dynamic, num_dynamic, pooler_resolution, return_intermediate):
        super().__init__()
    ...
    def forward(self, features, init_bboxes, init_features, image_shapes):
        ...
        for rcnn_head in self.head_series:
            class_logits, pred_bboxes, proposal_features = rcnn_head(features, bboxes, proposal_features, self.box_pooler, image_shapes)
...
class HungarianMatcher(nn.Module):
    def __init__(self, cost_class: float = 2.0, cost_bbox: float = 5.0, cost_giou: float = 2.0, focal_alpha=0.25, focal_gamma=2.0):
        super().__init__()
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        class_loss = sigmoid_focal_loss(
            src_logits, labels, num_boxes, alpha=self.focal_alpha, gamma=self.focal_gamma
        )
...
class SparseRCNNModel(nn.Module):
    def __init__(self, backbone, num_classes, num_proposals, hidden_dim, dim_feedforward, nhead, dropout, activation, 
                 num_heads, num_cls, num_reg, dim_dynamic, num_dynamic, pooler_resolution, deep_supervision):
        super().__init__()
    ...
    def forward(self, tensor):
        ...
        images_whwh = torch.tensor([w, h, w, h], dtype=torch.float32, device=tensor.device).unsqueeze(0).repeat(b, 1)
        image_shapes = [(h, w)] * b
        ...
        outputs_class, outputs_coord = self.head(features, proposal_boxes, self.init_proposal_features.weight, image_shapes)
        ...
        if self.model.training and targets is not None:
            ...
            for target, size in zip(targets, target_sizes):
                ...
                new_targets.append({
                    ...
                    "image_size_xyxy_tgt": image_size_xyxy.unsqueeze(0).repeat(len(gt_boxes_xyxy), 1)
                })
