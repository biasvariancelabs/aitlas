...
def selective_scan_easy(us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=False, return_last_state=False, chunksize=64):
    ...
    def selective_scan_chunk(us, dts, As, Bs, Cs, hprefix):
        ...
        scale = Ats[-1].detach().clamp(min=1e-30)
        rAts = Ats / scale
        duts = dts * us
...
class ChangeDecoder(nn.Module):
    ...
    def forward(self, pre_features, post_features):
        ...
        ct_tensor_42 = torch.stack([pre_feat_4, post_feat_4], dim=-1).reshape(B, C, H, 2 * W)
        p42 = self.st_block_42(ct_tensor_42)

        # 3. Parallel: Side-by-side concatenation (Left=Pre, Right=Post)
        ct_tensor_43 = torch.cat([pre_feat_4, post_feat_4], dim=-1)
