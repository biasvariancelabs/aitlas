...
def selective_scan_easy(
    us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=False, chunksize=64
):
    """
...
class SS2D(nn.Module):
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        x_hwwh = torch.stack(
            [x_flat, x_trans, x_flat.flip(-1), x_trans.flip(-1)], dim=1
        )  # (B, K=4, C, L)
...
class ChangeDecoder(nn.Module):
    ...
    * A "Smooth Layer" refines the result.
    ...
    def forward(self, pre_features, post_features):
        ...
        p42 = self.st_block_42(
            torch.stack([pre_feat_4, post_feat_4], dim=-1).reshape(B, C, H, 2 * W)
        )
        ...
        p32 = self.st_block_32(
            torch.stack([pre_feat_3, post_feat_3], dim=-1).reshape(B, C, H, 2 * W)
        )
        ...
        p22 = self.st_block_22(
            torch.stack([pre_feat_2, post_feat_2], dim=-1).reshape(B, C, H, 2 * W)
        )
        ...
        p12 = self.st_block_12(
            torch.stack([pre_feat_1, post_feat_1], dim=-1).reshape(B, C, H, 2 * W)
        )
...
class ChangeMamba(BaseChangeDetection):
    ...
    def __init__(self, config):
        ...
        self.model = ChangeMambaBCDModel(
            backbone_cfg=backbone_cfg, num_classes=self.config.num_classes
        )
