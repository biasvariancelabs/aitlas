...
class PoseResNet(nn.Module):
    ...
    def _get_deconv_cfg(self, deconv_kernel, index):
        if deconv_kernel == 4:
            ...
            raise ValueError(f"Unsupported deconv_kernel size: {deconv_kernel}")
...
class CenterNet(BaseObjectDetection):
    ...
    def load_centernet_weights(self, checkpoint_path):
        ...
        if msg.unexpected_keys:
            print(
                f"Unexpected keys ({len(msg.unexpected_keys)}): {msg.unexpected_keys[:10]}{'...' if len(msg.unexpected_keys) > 10 else ''}"
            )
    ...
    def forward(self, inputs, targets=None):
        ...
        if self.model.training and targets is not None:
            ...
            hm, wh, reg, ind, reg_mask = self.generate_targets(
                targets, output_h, output_w, self.device
            )
