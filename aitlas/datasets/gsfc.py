...
"""
27 reference cloud masks over NASA GSFC for various cloud conditions from September 2017 to November 2018. The masks were created
to validate cloud masking algorithms for Sentinel-2.
"""


class GSFCDataset(SemanticSegmentationDataset):
    url = "https://drive.google.com/drive/folders/1HaXiyJxm4CzvC__zLt2VQd3Qvb1WC9nF?usp=sharing"

    labels = ["clear", "cloud", "thin cloud", "shadow"]
    ...
    def load_dataset(self, data_dir, csv_file=None):
        ...
        self.imagesB01 = [
            os.path.join(data_dir, "B01", image_id[: image_id.rfind(".tif")] + "_B01.tif")
            for image_id in ids
        ]
        self.imagesB02 = [
            os.path.join(data_dir, "B02", image_id[: image_id.rfind(".tif")] + "_B02.tif")
            for image_id in ids
        ]
        self.imagesB03 = [
            os.path.join(data_dir, "B03", image_id[: image_id.rfind(".tif")] + "_B03.tif")
            for image_id in ids
        ]
        self.imagesB04 = [
            os.path.join(data_dir, "B04", image_id[: image_id.rfind(".tif")] + "_B04.tif")
            for image_id in ids
        ]
        self.imagesB05 = [
            os.path.join(data_dir, "B05", image_id[: image_id.rfind(".tif")] + "_B05.tif")
            for image_id in ids
        ]
        self.imagesB06 = [
            os.path.join(data_dir, "B06", image_id[: image_id.rfind(".tif")] + "_B06.tif")
            for image_id in ids
        ]
        self.imagesB07 = [
            os.path.join(data_dir, "B07", image_id[: image_id.rfind(".tif")] + "_B07.tif")
            for image_id in ids
        ]
        self.imagesB08 = [
            os.path.join(data_dir, "B08", image_id[: image_id.rfind(".tif")] + "_B08.tif")
            for image_id in ids
        ]
        self.imagesB8A = [
            os.path.join(data_dir, "B8A", image_id[: image_id.rfind(".tif")] + "_B8A.tif")
            for image_id in ids
        ]
        self.imagesB09 = [
            os.path.join(data_dir, "B09", image_id[: image_id.rfind(".tif")] + "_B09.tif")
            for image_id in ids
        ]
        self.imagesB10 = [
            os.path.join(data_dir, "B10", image_id[: image_id.rfind(".tif")] + "_B10.tif")
            for image_id in ids
        ]
        self.imagesB11 = [
            os.path.join(data_dir, "B11", image_id[: image_id.rfind(".tif")] + "_B11.tif")
            for image_id in ids
        ]
        self.imagesB12 = [
            os.path.join(data_dir, "B12", image_id[: image_id.rfind(".tif")] + "_B12.tif")
            for image_id in ids
        ]
    ...
    def show_image_all_bands_data(self, index, show_title=False):
        img, mask = self[index]
