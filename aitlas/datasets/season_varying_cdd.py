...
"""
As a common dataset for change detection, the dataset contains three parts: the image before the 
change, the image after the change, and the label image showing the changed area. This dataset is 
characterized by significant seasonal differences between bi-temporal image pairs. The labels for 
this dataset include some irregular changes, such as the appearance and disappearance of cars; but 
do not include seasonal changes, such as changes in the ground surface caused by snowfall.
"""

class SeasonVaryingCDDDataset(BaseDataset):

    url = "https://drive.google.com/file/d/1GX656JqqOyBi_Ef0w65kDGVto-nHrNs9"
    name = "Season-varying Change Detection Dataset"
    schema = FAIREOSchema
    ...
    def __getitem__(self, index):
        ...
        pre_image = image_loader(os.path.join(self.data_dir, 'A', f'{img_id}.jpg'))
        ...
        post_image = image_loader(os.path.join(self.data_dir, 'B', f'{img_id}.jpg'))
        #normalize image 
        img_min, img_max = post_image.min(), post_image.max()
        post_image = (post_image - img_min) / (img_max - img_min + 1e-8)

        mask = image_loader(os.path.join(self.data_dir, 'OUT', f'{img_id}.jpg'))
