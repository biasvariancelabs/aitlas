...
"""
The EGY-BCD dataset contains high-resolution and multitemporal satellite images captured in four urban 
and coastal areas in Egypt to detect building changes.
"""

class EGYBCDDataset(BaseDataset):

    url = "https://github.com/oshholail/EGY-BCD"
    name = "EGY-BCD"
    schema = FAIREOSchema
    ...
    def load_dataset(self, data_dir, csv_file):
        ...
        if csv_file: # train/val/test splits provided by BVLabs
            self.data = []
            with open(csv_file, mode ='r')as file:
                csvFile = csv.reader(file)
                for lines in csvFile:
                    self.data.append(lines[0].split('/')[1][: -4])
