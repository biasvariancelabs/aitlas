...
'''
WHDLD (Wuhan Dense Labeling Dataset) is a dense labeling dataset that can be used for multi-label tasks 
such as remote sensing image retrieval (RSIR) and classification, and other pixel-based tasks like semantic 
segmentation. This dataloader considers the multi-label subset.
'''

LABELS = ["building","road","pavement","vegetation","baresoil","water"]


class WHDLDMultilabelDataset(MultiLabelClassificationDataset):
    url = "https://sites.google.com/view/zhouwx/dataset"

    labels = LABELS
    name = "WHDLD multilabel"
