...
class SpaceNet6TrainAndEvaluateTask(BaseTask):
    ...
    def run(self):
        for fold in [0, 3, 6, 9, 1, 2, 7, 8]:
            ...
            train_dataset.load_training_image_filenames(self.config.fold_path, fold, self.config.root_directory,
            ...
            if self.config.apex:
                ...
                self.model, optimizer = amp.initialize(self.model.cuda(), optimizer, opt_level="O1")
            self.model.cuda()
            queue = Queue()
            best_f1_score = -1
            # Kick off training
            for epoch in range(self.config.epochs if self.config.train else 1):
                if self.config.train:
                    ...
                    for sample in iterator:
                        time1 = time.time()
                        load = time1 - time2
                        imgs = self.config.input_scale * sample["image"].cuda(non_blocking=True)
                        ...
                        coord = self.config.coord_scale * sample["coordinate"].cuda(non_blocking=True)
                        ...
                        if self.config.apex:
                            ...
                            torch.nn.utils.clip_grad_norm_(amp.master_params(optimizer),
                                                           self.config.clip_grad_norm_value)
                        else:
                            loss.backward()
                    ...
                    with torch.no_grad():
                        for _, sample in enumerate(tqdm(val_data_loader)):
                            ...
                            for i in range(len(o)):
                                ...
                                io.imsave(
                                    os.path.join(self.config.pred_folder, os.path.split(sample['image_path'][i])[1]),
                                    img)
                    torch.cuda.empty_cache()
                if self.config.val and epoch >= self.config.start_val_epoch:
                    ...
                    def new_thread():
                        ...
                        test_post_process(self.config.pred_folder, pred_csv)
                    ...
                    t.start()
        # Merge everything
        shutil.rmtree(self.config.merge_pred_dir, ignore_errors=True)
        os.makedirs(self.config.merge_pred_dir, exist_ok=True)
        flds = [0, 1, 2, 3, 6, 7, 8, 9]
        pred_folders = [self.config.pred_folder.format(i) for i in flds]
        for filename in tqdm(os.listdir(pred_folders[0])):
            used_masks = [io.imread(os.path.join(ff, filename)) for ff in pred_folders]
            mask = np.zeros_like(used_masks[0], dtype="float")
            for used_mask in used_masks:
                mask += used_mask.astype("float") / len(flds)
            io.imsave(os.path.join(self.config.merged_pred_dir, filename), mask)
        test_post_process(self.config.merged_pred_dir, self.config.solution_file)
