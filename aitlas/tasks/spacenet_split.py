...
class SpaceNetSplitTask(BaseTask):
    """
    Task for splitting the SpaceNet datasets into train, test and validation sets.
    Works equally for challenges 5 and 6.
    ...
    def __init__(self, model, config):
        ...
        self.filenames = [f for f in os.listdir(path=self.config.root) if f.endswith(".tif")]

    def is_valid_split(self):
        """Checks if the specified split is valid by ensuring that the sum of ratios is equal to 100%."""
    ...
    def run(self):
        ...
        split_indices = random_split(dataset=range(dataset_size),
                                     lengths=dataset_lengths,
                                     generator=torch.Generator().manual_seed(42))
        # Save train and test split
        self.save_split(split_indices=split_indices[0], destination_file=self.config.split.train.file)
        self.save_split(split_indices=split_indices[1], destination_file=self.config.split.test.file)
        # Save validation split
        if self.has_validation_dataset():
            self.save_split(split_indices=split_indices[2], destination_file=self.config.split.val.file)