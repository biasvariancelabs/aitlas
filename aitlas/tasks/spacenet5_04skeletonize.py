...
from aitlas.base import Config, BaseTask
...
def preprocess(image, thresh, image_multiplier=255, hole_size=300, cv2_kernel_close=7, cv2_kernel_open=7,
               verbose=False):
    """
...
def remove_small_terminal(g, weight='weight', min_weight_val=30, pix_extent=1300, edge_buffer=4, verbose=False):
    """
        ...
        for p_tmp in [sx, sy, ex, ey]:
            if (p_tmp < (0 + edge_buffer)) or (p_tmp > (pix_extent - edge_buffer)):
                if verbose:
                    ...
                    print("(p_tmp > (pix_extent - edge_buffer):", (p_tmp > (pix_extent - edge_buffer)))
...
def add_small_segments(g, terminal_points, terminal_lines, dist1=24, dist2=80, angle1=30, angle2=150, verbose=False):
    """
...
def make_skeleton(image_location, thresh, fix_borders, replicate=5, clip=2, img_shape=(1300, 1300),
                  image_multiplier=255, hole_size=300, cv2_kernel_close=7, cv2_kernel_open=7, use_medial_axis=False,
                  max_out_size=(200000, 200000), num_classes=1, skeleton_band='all', verbose=False):
    """
    ...
    t1 = time.time()
    img = preprocess(img, thresh, image_multiplier=image_multiplier, hole_size=hole_size,
...
def image_to_skeleton_graph(params):
    img_loc, out_ske_file, out_g_pickle, thresh, debug, fix_borders, img_shape, skeleteon_replicate, skeleton_clip, \
    image_multiplier, hole_size, cv2_kernel_close, cv2_kernel_open, min_subgraph_length_pix, min_spur_length_pix, \
    max_out_size, use_medial_axis, num_classes, skeleton_band, kernel_blur, min_background_frac, verbose = params
    # Create skeleton
    img_refine, ske = make_skeleton(image_location=img_loc, thresh=thresh, fix_borders=fix_borders,
                                    replicate=skeleteon_replicate, clip=skeleton_clip, img_shape=img_shape,
                                    image_multiplier=image_multiplier, hole_size=hole_size,
                                    cv2_kernel_close=cv2_kernel_close, cv2_kernel_open=cv2_kernel_open,
                                    max_out_size=max_out_size, skeleton_band=skeleton_band, num_classes=num_classes,
                                    use_medial_axis=use_medial_axis, verbose=verbose)
...
def build_wkt_dir(in_directory, outfile, out_ske_dir, out_g_dir='', thresh=0.3, debug=False,
                  add_small=True, fix_borders=True, image_shape=(1300, 1300), skeleton_replicate=5, skeleton_clip=2,
                  image_multiplier=255, hole_size=300, cv2_kernel_close=7, cv2_kernel_open=7,
                  min_subgraph_length_pix=50, min_spur_length_pix=16, space_net_naming_convention=False, num_classes=1,
                  max_out_size=(100000, 100000), use_medial_axis=True, skeleton_band='all', kernel_blur=27,
                  min_background_fraction=0.2, n_threads=12, verbose=False):
    """
    ...
    for i, image_file in enumerate(image_files):
        ...
        if len(out_g_dir) > 0:
            out_g_pickle = os.path.join(out_g_dir, image_file.split('.')[0] + '.gpickle')
        else:
            out_g_pickle = ''
        param_row = (img_loc, out_ske_file, out_g_pickle, thresh, debug, fix_borders, image_shape, skeleton_replicate,
                     skeleton_clip, image_multiplier, hole_size, cv2_kernel_close, cv2_kernel_open,
                     min_subgraph_length_pix, min_spur_length_pix, max_out_size, use_medial_axis, num_classes,
                     skeleton_band, kernel_blur, min_background_fraction, verbose)
...
class SpaceNet5SkeletonizeTask(BaseTask):
    ...
    def run(self):
        ...
        n_threads = 12
        min_spur_length_pix = int(np.rint(self.config.min_spur_length_m / self.config.GSD))
        ...
        out_ske_dir = os.path.join(res_root_dir, self.config.skeleton_dir)  # set to '' to not save
        os.makedirs(out_ske_dir, exist_ok=True)
        if len(self.config.skeleton_pkl_dir) > 0:
            out_g_dir = os.path.join(res_root_dir, self.config.skeleton_pkl_dir)  # set to '' to not save
        ...
        t0 = time.time()
        df = build_wkt_dir(self.config.masks_dir, outfile_csv, out_ske_dir, out_g_dir, thresh, debug=debug,
                           add_small=add_small, fix_borders=fix_borders, image_shape=img_shape,
                           skeleton_replicate=skeleton_replicate, skeleton_clip=skeleton_clip,
                           image_multiplier=image_multiplier, hole_size=hole_size,
                           min_subgraph_length_pix=min_subgraph_length_pix, min_spur_length_pix=min_spur_length_pix,
                           cv2_kernel_close=cv2_kernel_close, cv2_kernel_open=cv2_kernel_open,
                           max_out_size=max_out_size, skeleton_band=self.config.skeleton_band,
                           num_classes=self.config.num_classes, space_net_naming_convention=space_net_naming_convention,
                           use_medial_axis=use_medial_axis, kernel_blur=kernel_blur, n_threads=n_threads,
                           min_background_fraction=min_background_fraction, verbose=verbose)
        print("len df:", len(df))
