...
def convert_to_8bit(input_raster, output_raster, output_pix_type="Byte", output_format="GTiff", rescale_type="perc",
                    percentiles=None, band_order=None, no_data_value=0, max_zero_fraction=0.3):
    """
    ...
    if n_bands == 3:
        cmd = ['gdal_translate', '-ot', output_pix_type, '-of', output_format, '-a_nodata', str(no_data_value), '-co',
               '"PHOTOMETRIC=rgb"']
    else:
        cmd = ['gdal_translate', '-ot', output_pix_type, '-of', output_format, '-a_nodata', str(no_data_value)]
    # Get bands
    if len(band_order) == 0:
...
def prepare_image(params):
    im_file, im_file_raw, im_file_out, output_pix_type, output_format, \
    rescale_type, percentiles, max_zero_fraction, band_order = params
    if not im_file.endswith('.tif'):
        return
    if not os.path.isfile(im_file_out):
        convert_to_8bit(im_file_raw, im_file_out, output_pix_type=output_pix_type, output_format=output_format,
                        rescale_type=rescale_type, percentiles=percentiles, band_order=band_order,
...
class SpaceNet5PrepareImageryTask(BaseTask):
    ...
    def run(self):
        ...
        for i, im_file in enumerate(im_files):
            ...
            params.append((im_file, im_file_raw, im_file_out, output_pix_type, output_format, self.config.rescale_type,
                           percentiles, max_zero_fraction, band_order))
