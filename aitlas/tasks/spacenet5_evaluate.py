...
class SpaceNet5EvaluateTask(BaseTask):
    ...
    def run(self):
        ...
        # Run evaluation
        with torch.no_grad():
            for i, data in enumerate(tqdm(dataset.dataloader(), desc="[custom evaluation]")):
                ...
                for metric_class in metrics:
                    ...
                    calculated_metrics[metric.name] += metric.calculate(y_true=target_mask, y_pred=output_mask)
