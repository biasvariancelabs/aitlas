...
def graph_to_geo_dfs_pix(g, nodes=True, edges=True, node_geometry=True, fill_edge_geometry=True):
    """
...
def plot_graph_pix(g, image=None, bbox=None, fig_height=6, fig_width=None, margin=0.02, axis_off=True,
                   equal_aspect=False, bg_color='w', show=True, save=False, close=True, file_format='png',
                   filename='temp', default_dpi=300, annotate=False, node_color='#66ccff', node_size=15, node_alpha=1,
                   node_edge_color='none', node_z_order=1, edge_color='#999999', edge_line_width=1, edge_alpha=1,
                   edge_color_key='speed_mph', color_dict=None, edge_width_key='speed_mph',
                   edge_width_multiplier=1. / 25, use_geom=True, invert_x_axis=False, invert_y_axis=False, fig=None,
                   ax=None):
    """
    ...
    lc = LineCollection(lines, colors=edge_colors, linewidths=widths, alpha=edge_alpha, zorder=2)
    ax.add_collection(lc)
    log('Drew the graph edges in {:,.2f} seconds'.format(time.time() - start_time))
    # Scatter plot the nodes
    ax.scatter(node_xs, node_ys, s=node_size, c=node_color, alpha=node_alpha, edgecolor=node_edge_color,
               zorder=node_z_order)
    ...
    if equal_aspect:
        ...
        if g.graph['crs'] == ox_settings.default_crs:
            cos_lat = np.cos((min(node_ys) + max(node_ys)) / 2. / 180. * np.pi)
    ...
    fig, ax = save_and_show(fig, ax, save, show, close, filename, file_format, dpi, axis_off,
                            invert_x_axis=invert_x_axis, invert_y_axis=invert_y_axis)
    return fig, ax


def plot_graph_route_pix(g, route, image=None, bbox=None, fig_height=6, fig_width=None, margin=0.02, bg_color='w',
                         axis_off=True, show=True, save=False, close=True, file_format='png', filename='temp',
                         default_dpi=300, annotate=False, node_color='#999999', node_size=15, node_alpha=1,
                         node_edge_color='none', node_z_order=1, edge_color='#999999', edge_line_width=1, edge_alpha=1,
                         edge_color_key='speed_mph', color_dict=None, edge_width_key='speed_mph',
                         edge_width_multiplier=1. / 25, use_geom=True, origin_point=None, destination_point=None,
                         route_color='r', route_line_width=4, route_alpha=0.5, orig_destination_node_alpha=0.5,
                         orig_destination_node_size=100, orig_destination_node_color='r', invert_x_axis=False,
                         invert_y_axis=True, fig=None, ax=None):
    """
    ...
    fig, ax = plot_graph_pix(g, image=image, bbox=bbox, fig_height=fig_height, fig_width=fig_width, margin=margin,
                             axis_off=axis_off, bg_color=bg_color, show=False, save=False, close=False,
                             filename=filename, default_dpi=default_dpi, annotate=annotate, node_color=node_color,
                             node_size=node_size, node_alpha=node_alpha, node_edge_color=node_edge_color,
                             node_z_order=node_z_order, edge_color_key=edge_color_key, color_dict=color_dict,
                             edge_color=edge_color, edge_line_width=edge_line_width, edge_alpha=edge_alpha,
                             edge_width_key=edge_width_key, edge_width_multiplier=edge_width_multiplier,
                             use_geom=use_geom, fig=fig, ax=ax)
    ...
    if origin_point is None or destination_point is None:
        ...
        origin_destination_ys = (g.nodes[origin_node]['y_pix'], g.nodes[destination_node]['y_pix'])
        origin_destination_xs = (g.nodes[origin_node]['x_pix'], g.nodes[destination_node]['x_pix'])
    else:
    ...
    ax.scatter(origin_destination_xs, origin_destination_ys, s=orig_destination_node_size,
               c=orig_destination_node_color, alpha=orig_destination_node_alpha, edgecolor=node_edge_color, zorder=4)
    ...
    for u, v in edge_nodes:
        ...
        if 'geometry_pix' in data and use_geom:
            ...
            lines.append(line)
    # Add the lines to the axis as a LineCollection
    lc = LineCollection(lines, colors=route_color, linewidths=route_line_width, alpha=route_alpha, zorder=3)
    ax.add_collection(lc)
    # Update dpi, if image
    if image is not None:
        # mpl can handle a max of 2^29 pixels, or 23170 on a side
    ...
    # Save and show the figure as specified
    fig, ax = save_and_show(fig, ax, save, show, close, filename, file_format, dpi, axis_off,
                            invert_y_axis=invert_y_axis, invert_x_axis=invert_x_axis)
    return fig, ax


def save_and_show(fig, ax, save, show, close, filename, file_format, dpi, axis_off, tight_layout=False,
                  invert_x_axis=False, invert_y_axis=True, verbose=False):
    """
        ...
        if file_format == 'svg':
            ...
            if tight_layout:
                ...
                fig.savefig(path_filename, dpi=dpi, bbox_inches=extent, format=file_format,
                            facecolor=fig.get_facecolor(), transparent=True, pad_inches=0)
            else:
                fig.savefig(path_filename, dpi=dpi, bbox_inches=extent, format=file_format,
        ...
        for i, im_root in enumerate(im_list):
            ...
            if not save_only_route_png:
                ...
                plot_graph_pix(G, im, fig_height=fig_height, fig_width=fig_width, node_size=int(node_size),
                               ...
                               edge_alpha=edge_alpha, edge_color=edge_color, filename=out_file_plot, default_dpi=dpi,
                               edge_color_key="", show=False, save=True, invert_y_axis=invert_y_axis,
