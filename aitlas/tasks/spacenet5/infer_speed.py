...
def plot_graph_on_im_yuge(
    ...
    if h > w:
        fig_size = (fig_size[1] * 1.0 * w / h, fig_size[1])
    elif w > h:
        fig_size = (fig_size[0], fig_size[0] * 1.0 * h / w)
    else:
        pass
    ...
    lc = mpl_collections.LineCollection(
        lines, colors=edge_color, linewidths=widths, alpha=line_alpha
    )
...
def get_linestring_midpoints(geom):
    ...
    for i in range(N - 1):
        ...
        dls.append(dl)
    return (
        np.array(x_middles).astype(int),
        np.array(y_middles).astype(int),
        np.array(dls),
    )
...
def get_patch_speed_multichannel(
    patch,
    conv_dict,
    min_z=128,
    weighted=True,
...
def get_edge_time_properties(
    ...
    for j, (x, y, dl_pix) in enumerate(zip(x_mids, y_mids, dls)):
        ...
        if multichannel:
            ...
            speed_mph_seg, z = get_patch_speed_multichannel(
                patch,
                conv_dict,
                percentile=percentile,
                min_z=min_z,
                weighted=use_weighted_mean,
                verbose=verbose,
            )
        else:
            patch = mask[y0:y1, x0:x1]
            z = 0
            speed_mph_seg, _ = get_patch_speed_single_channel(
                patch, conv_dict, percentile=percentile, verbose=verbose
            )
        # Add to arrays
...
def infer_travel_time(params):
    ...
    for i, (u, v, edge_data) in enumerate(g_.edges(data=True)):
        ...
        tot_hours, mean_speed_mph, length_miles = get_edge_time_properties(
            mask,
            edge_data,
            conv_dict,
            min_z=min_z,
    ...
    if save_geo_packages:
        ...
        filename = (
            im_root + ".gpkg"
        )  # append GeoPackage extension to the filename so it doesn't raise warnings
...
def infer_travel_time_single_threaded(
    g_,
    mask,
    conv_dict,
    min_z=128,
...
class SpaceNet5InferSpeedTask(BaseTask):
    ...
    def run(self):
        ...
        if self.config.num_classes > 1:
            conv_df, conv_dict = load_speed_conversion_dict_binned(
                speed_conversion_file
            )
        else:
            conv_df, conv_dict = load_speed_conversion_dict_continuous(
                speed_conversion_file
            )
        ...
        add_travel_time_dir(
            ...
            use_weighted_mean=use_weighted_mean,
            variable_edge_speed=variable_edge_speed,
            save_geopackages=save_geopackages,
            n_threads=n_threads,
            verbose=verbose,
        )
        t1 = time.time()
        ...
        if n_plots > 0:
            ...
            # Define output dir
            graph_speed_plots_dir = os.path.join(
                res_root_dir, self.config.graph_dir + "_speed_plots"
            )
            ...
            for i, image_name in enumerate(image_names):
                ...
                pkl_path = os.path.join(
                    graph_speed_dir, image_name.split(".")[0] + ".gpickle"
                )
                ...
                _ = plot_graph_on_im_yuge(
                    ...
                    width_multiplier=plot_width_multiplier,
