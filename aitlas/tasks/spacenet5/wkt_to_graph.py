...
def get_node_geo_coordinates(
    g, im_file, fix_utm_zone=True, n_threads=12, verbose=False
):
    # Get pixel params
    params = []
    nn = len(g.nodes())
...
def get_edge_geo_coordinates(
    ...
    for i, (u, v, attr_dict) in enumerate(g.edges(data=True)):
        geom_pix = attr_dict["geometry_pix"]
        l_string_lat_lon, l_string_utm, utm_zone, utm_letter = coordinate_dict[(u, v)][
            0
        ]
...
def wkt_to_g(params):
    ...
    if im_file:
        ...
        g1 = get_node_geo_coordinates(
            g1, im_file, n_threads=n_threads_tmp, verbose=verbose
        )
        t4 = time.time()
        if verbose:
        ...
        for i, (u, v, attr_dict) in enumerate(g_projected.edges(data=True)):
            if "geometry" in attr_dict.keys():
                attr_dict["geometry_wkt"] = attr_dict[
                    "geometry"
                ].wkt  # attr_dic['geometry'] key error
        if verbose:
    ...
    if simplify_graph:
        ...
        if verbose:
            ...
            print(
                edge_tmp,
                "random edge props:",
                g_out.get_edge_data(edge_tmp[0], edge_tmp[1]),
            )
        t8 = time.time()
        if verbose:
