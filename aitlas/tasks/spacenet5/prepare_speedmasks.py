...
def gdf_to_array(
    ...
    if compress:
        ...
        target_ds = gdal.GetDriverByName("GTiff").Create(
            output_raster, gdata.RasterXSize, gdata.RasterYSize, 1, gdal.GDT_Byte
        )
    ...
    out_layer = out_data_source.CreateLayer(
        "states_extent", raster_srs, geom_type=ogr.wkbMultiPolygon
    )
    # Burn
    burn_field = "burn"
    ...
    if len(mask_burn_value_key) > 0:
        gdal.RasterizeLayer(
            target_ds, [1], out_layer, options=["ATTRIBUTE=%s" % burn_field]
        )
    else:
        gdal.RasterizeLayer(target_ds, [1], out_layer, burn_values=[burn_value])


def create_speed_gdf(
    image_path,
    geojson_path,
    mask_path_out_gray,
    bin_conversion_function,
    mask_burn_value_key="burnValue",
    ...
    gdf_utm_buffer["geometry"] = gdf_utm_buffer.buffer(
        buffer_distance_meters, buffer_roundness
    )
    ...
    gdf_to_array(
        ...
        image_path,
        mask_path_out_gray,
        mask_burn_value_key=mask_burn_value_key,
        verbose=verbose,
    )
    ...
    try:
        ...
        zero_fraction = 1.0 - float(np.count_nonzero(im_gray)) / im_gray.size
        if zero_fraction >= zero_fraction_threshold:
            print("zero_fraction:", zero_fraction)
            print(
                "create_speed_gdf(): checking to ensure masks are null where image is null"
            )
...
def convert_array_to_multichannel(
    in_arr, n_channels=7, burn_value=255, append_total_band=False, verbose=False
):
    """
...
def speed_masks(
    ...
    for j, image_name in enumerate(images):
        ...
        geojson_path = os.path.join(
            geojson_dir,
            image_root.replace("PS-RGB", "geojson_roads_speed").replace(
                "PS-MS", "geojson_roads_speed"
            )
            + ".geojson",
        )
        if (j % 1) == 0:
            print(
                j + 1, "/", len(images), "image:", image_name, "geojson:", geojson_path
            )
        if j > 0:
            verbose = False
        create_speed_gdf(
            ...
            mask_path_out,
            speed_to_burn_func,
            mask_burn_value_key=mask_burn_val_key,
...
class SpaceNet5PrepareSpeedMasksTask(BaseTask):
    ...
    def run(self):
        ...
        if len(self.config.output_mask_multidim_dir) == 0:
            ...
            def speed_to_burn_value(speed):
                ...
                burn_val = min(
                    ...
                    + bw
                    * (
                        (speed - min_speed_continuous)
                        / (max_speed_continuous - min_speed_continuous)
                    ),
                    mask_max,
                )
                return max(burn_val, min_road_burn_val)

            speed_array_values = np.arange(
                min_speed_continuous, max_speed_continuous + 1, 1
            )
            burn_values_array = [speed_to_burn_value(s) for s in speed_array_values]
            df = pd.DataFrame(
                {"burn_value": burn_values_array, "speed_value": speed_array_values}
            )
        # Multi-channel case
        else:
            min_speed_bin = 1
            ...
            def speed_to_burn_value(speed_mph):
                ...
                """
                return int(
                    int(math.ceil(speed_mph / bin_size_mph)) * channel_value_multiplier
                )

            n_channels = len(
                np.unique([int(speed_to_burn_value(z)) for z in speed_arr_bin])
            )
            channel_value_multiplier = int(255 / n_channels)
            # Make conversion dataframe
            burn_values_array = np.array(
                [speed_to_burn_value(s) for s in speed_arr_bin]
            )
            df = pd.DataFrame(
                {"burn_value": burn_values_array, "speed_value": speed_arr_bin}
            )
            # Add a channel column, for the channel that the speed corresponds to
            channel_value = (burn_values_array / channel_value_multiplier).astype(
                int
            ) - 1
        ...
        if not os.path.exists(self.config.output_conversion_csv):
            print(
                f"Writing speed<->burn conversion to: {self.config.output_conversion_csv}"
            )
            df.to_csv(self.config.output_conversion_csv)
        else:
            print(
                f"Path {self.config.output_conversion_csv} for speed<->burn conversion already exists"
            )
        speed_masks(
            self.config.geojson_dir,
            self.config.image_dir,
            self.config.output_mask_contin_dir,
            speed_to_burn_value,
            mask_burn_val_key=mask_burn_val_key,
            buffer_distance_meters=self.config.buffer_distance_meters,
            buffer_roundness=buffer_roundness,
            dissolve_by=dissolve_by,
            bin_conversion_key=bin_conversion_key,
            verbose=verbose,
            output_dir_multidim=self.config.output_mask_multidim_dir,
            channel_value_multiplier=channel_value_multiplier,
            n_channels=n_channels,
            channel_burn_value=channel_burn_value,
            append_total_band=append_total_band,
        )
