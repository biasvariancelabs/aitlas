...
from aitlas.models import SparseRCNN


@pytest.fixture(scope="module")
def sparsercnn_model():
    """
    A pytest fixture that instantiates the SparseRCNN once per test module.
    """
    mock_config = {
        "pretrained": True,
        "num_classes": 20,
    ...
    try:
        ...
        model = SparseRCNN(mock_config)
        return model
    except Exception as e:
        pytest.fail(f"Failed to instantiate SparseRCNN: {e}")


def test_sparsercnn_eval_forward_pass(sparsercnn_model):
    """
    Test the forward pass of the SparseRCNN model in evaluation mode.
    """
    sparsercnn_model.model.eval()
    batch_size = 2
    channels = 3
    height = 512
    width = 512
    ...
    try:
        ...
        outputs = sparsercnn_model(inputs)
...
def test_sparsercnn_train_forward_pass(sparsercnn_model):
    """
    Test the forward pass of the SparseRCNN model in training mode.
    """
    sparsercnn_model.model.train()
    batch_size = 2
    channels = 3
    height = 512
    width = 512
    ...
    try:
        ...
        outputs = sparsercnn_model(inputs, targets)
