...
class MockCopernicusBackbone(nn.Module):
    ...
    def forward_features(self, x, metadata, wavelengths=None, bandwidths=None, language_embed=None, input_mode='spectral', kernel_size=None):
        """
...
@patch('aitlas.models.copernicusfm_wrapper.copernicusfm_base', side_effect=MockCopernicusBackbone)
def test_instantiation_copernicus_local(mock_backbone_cls, mock_os_exists, mock_torch_load, config_copernicus_base):
    ...
    mock_torch_load.assert_called_with(config_copernicus_base["local_model_path"], weights_only=False)
...
def test_instantiation_copernicus_download(
    mock_backbone_cls, mock_os_exists, mock_torch_load, mock_hf_download, config_copernicus_base
):
...
def test_forward_pass_copernicus_spectral(
    ...
    config_copernicus_base, dummy_copernicus_input_spectral
):
...
def test_forward_pass_copernicus_variable(
    mock_backbone_cls, mock_os_exists, mock_torch_load, 
    config_copernicus_base, dummy_copernicus_input_variable
):
