import pytest
import torch
from aitlas.models import EfficientDet

@pytest.fixture(scope="module")
def efficientdet_model():
    """
    A pytest fixture that instantiates the EfficientDet once per test module.
    """
    mock_config = {
        "pretrained": True,
        "num_classes": 20,
    ...
    try:
        ...
        model = EfficientDet(mock_config)
        return model
    except Exception as e:
        pytest.fail(f"Failed to instantiate EfficientDet: {e}")

def test_efficientdet_eval_forward_pass(efficientdet_model):
    """
    Test the forward pass of the EfficientDet model in evaluation mode.
    """
    efficientdet_model.model.eval()
    batch_size = 2
    channels = 3
    height = 512
    width = 512
    ...
    try:
        ...
        outputs = efficientdet_model(inputs)
...
def test_efficientdet_train_forward_pass(efficientdet_model):
    """
    Test the forward pass of the EfficientDet model in training mode.
    """
    efficientdet_model.model.train()
    batch_size = 2
    channels = 3
    height = 512
    width = 512
    ...
    try:
        ...
        outputs = efficientdet_model(inputs, targets)
