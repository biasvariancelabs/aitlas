import pytest
import torch
from aitlas.models import CenterNet

@pytest.fixture(scope="module")
def centernet_model():
    """
    A pytest fixture that instantiates the CenterNet once per test module.
    """
    mock_config = {
        "pretrained": True,
        "num_classes": 20,
    ...
    try:
        ...
        model = CenterNet(mock_config)
        return model
    except Exception as e:
        pytest.fail(f"Failed to instantiate CenterNet: {e}")

def test_centernet_eval_forward_pass(centernet_model):
    """
    Test the forward pass of the CenterNet model in evaluation mode.
    """
    centernet_model.model.eval()
    batch_size = 2
    channels = 3
    height = 512
    width = 512
    ...
    try:
        ...
        outputs = centernet_model(inputs)
...
def test_centernet_train_forward_pass(centernet_model):
    """
    Test the forward pass of the CenterNet model in training mode.
    """
    centernet_model.model.train()
    batch_size = 2
    channels = 3
    height = 512
    width = 512
    ...
    try:
        ...
        outputs = centernet_model(inputs, targets)
